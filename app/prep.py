...
BASE_URL = "https://raw.githubusercontent.com/AdairPonceuwu/ch_llm/main"
...
def fetch_documents():
    ...
    relative_url = "data_json/documents-with-ids.json"
    docs_url = f"{BASE_URL}/{relative_url}?raw=1"
    docs_response = requests.get(docs_url)
    docs_response.raise_for_status()  # Raise an exception for HTTP errors
        
    # Establecer la codificación a UTF-8
    docs_response.encoding = 'utf-8'
...
def fetch_ground_truth():
    ...
    relative_url = "data_csv/ground-truth-data.csv"
    ground_truth_url = f"{BASE_URL}/{relative_url}?raw=1"
    df_ground_truth = pd.read_csv(ground_truth_url)
    df_ground_truth = df_ground_truth[
        df_ground_truth.topic == "PMD"
    ]
